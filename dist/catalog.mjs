...
export const GUILDS=[
 {id:'explorers',name:'Explorers Guild',color:'#8fe6d4',desc:'Chart unknown stars and the stories beneath them.',quests:[quest('A wider lens','Scan 3 new worlds after accepting.','surveys',3,'cartographer',900),quest('Beyond the lanterns','Complete discovery scans in 6 new systems in the Uncharted Reach.','discoveries',6,'pathfinder',1600),quest('Signals from below','Record 5 new surface anomalies.','anomalies',5,'deepfield',2400)]},
...
export function expandGalaxy(systems,rng){
 ...
 for(let id=24;id<192;id++){const r=rng(7731+id*127),u=id>=64,n=id-64;r();r();systems.push({id,name:u?['Silent','Distant','Hollow','Silver','Faint','Hidden','Drifting','Ancient'][n%8]+' '+['Harbor','Echo','Veil','Crown','Warden','Garden','Ember','Current','Signal','Beacon','Promise','Tide','Atlas','Horizon','Orbit','Dawn'][Math.floor(n/8)]:names[id-24],x:0,y:0,eco:['Agricultural','Extraction','Industrial','Research','Frontier'][id%5],danger:u?1+Math.floor(r()*3):id%4,station:u?'Expedition relay '+(Math.floor(n/16)+1):['Haven','Exchange','Anchorage','Terminal'][id%4]+' '+(id+1),color:['#ecc095','#a8d7fc','#f59c71','#ccbcff','#e9debb'][id%5],lore:u?'Beyond the shipping lanes, each signal could be a discovery.':'Settlements and freight routes connect this part of the frontier.',uncharted:u,hasStation:!u||n%16===0,faction:u?null:FACTIONS[Math.floor(id/4)%3].id,catalog:u?'UR-'+String(n+1).padStart(3,'0'):null});}
 // Assign positions by distance from Solace. IDs and all local-system seeds stay stable.
 const sites=[];
 for(let q=-10;q<=10;q++)for(let v=-10;v<=10;v++){
  const x=8*(q+v/2),y=8*Math.sqrt(3)/2*v;
  sites.push({x,y,d:q*q+q*v+v*v,a:Math.atan2(y,x)});
 }
 sites.sort((a,b)=>a.d-b.d||a.a-b.a);
