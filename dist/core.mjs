...
export class Game {
 ...
 missionReady(m){return m.type==='delivery'?this.s.system===m.destination:this.s.system===m.origin&&(m.type==='survey'?[0,1].every(n=>this.s.scanned.includes(`planet-${m.destination}-${n}`)):m.type==='bounty'?(this.s.metrics?.pirates??this.s.kills)-m.startKills>=2:this.s.cargo.ore>=m.tons);}
 ...
 update(dt,input={}){
  ...
  for(const e of this.enemies){const d=dist(p,e);const safe=this.sys.hasStation!==false&&dist(p,this.station)<470;if(d<950&&!safe){e.angle=Math.atan2(p.y-e.y,p.x-e.x);const speed=d>210?100+this.sys.danger*10:d<150?-65:15;e.thrust=speed>0?.45:0;e.x+=Math.cos(e.angle)*speed*dt;e.y+=Math.sin(e.angle)*speed*dt;e.fire-=dt;if(e.fire<0&&d<600){e.fire=1.1+Math.random()*.6;this.shots.push({x:e.x,y:e.y,vx:Math.cos(e.angle)*330,vy:Math.sin(e.angle)*330,life:2,enemy:true,damage:9+this.sys.danger*2});this.onHostileAttack?.(e,p);}}else{e.thrust=.2;e.angle+=dt*.28;e.x+=Math.cos(e.angle)*26*dt;e.y+=Math.sin(e.angle)*26*dt;}}
  for(const b of this.shots){const from={x:b.x,y:b.y};b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(b.trafficShot)continue;if(b.enemy){if(segmentDistance(p,from,b)<p.r+7){let dam=b.damage,absorb=Math.min(this.s.shield,dam);this.s.shield-=absorb;this.s.hull-=dam-absorb;this.lastDamage=this.time;b.life=0;this.burst(p.x,p.y,'#ef8b7f',6);}}else{for(const t of [...this.enemies,...this.asteroids]){if(t.hp>0&&segmentDistance(t,from,b)<t.r+5){if(t.type==='enemy'&&b.playerShot)t.playerHit=true;t.hp-=b.damage;b.life=0;this.burst(b.x,b.y,t.type==='enemy'?'#f8a77f':'#8dddcc',5);if(t.hp<=0){const playerCredit=t.type==='enemy'&&!!t.playerHit;this.burst(t.x,t.y,'#ffd297',24);this.onDestroyed?.(t,playerCredit);if(t.type==='enemy'){if(playerCredit){this.s.kills++;this.s.credits+=t.bounty;this.notify(`Bounty confirmed · +${t.bounty} cr`,'good');}else this.notify(t.name+' destroyed by system security. No bounty awarded.');}else{if(cargoUsed(this.s)<st.cargo){const amount=Math.min(st.miningYield,st.cargo-cargoUsed(this.s));this.s.cargo[t.ore]+=amount;this.s.mined+=amount;this.notify(`${t.ore==='ore'?'Titanium ore':'Void crystal'} collected · ${amount} t`,'good');}else this.notify('Hold full. Dock to sell your cargo.');}if(this.target?.id===t.id)this.target=this.belt;}break;}}}}
