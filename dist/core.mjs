...
export class Game {
 ...
 autopilot(){if(this.s.docked){this.notify('Launch before engaging autopilot.');return;}if(this.auto){this.auto=null;this.notify('Manual flight.');return;}this.auto=this.target;this.notify('Autopilot → '+this.target.name);}
  scanTarget(){if(this.scan||this.spectrumScan||this.jump)return false;if(this.s.docked){this.notify('Launch and approach a world to scan it.');return false;}const isBody=t=>t&&(t.type==='planet'||t.type==='moon');const p=isBody(this.target)?this.target:this.planets.reduce((a,b)=>dist(this.player,a)<dist(this.player,b)?a:b);this.target=p;const st=getStats(this.s),d=dist(this.player,p);const spectrumed=(this.s.spectrumScanned||[]).includes(p.id);if(this.s.scanned.includes(p.id)){this.notify(spectrumed?'Body dossier complete. Already surveyed.':'This world is already surveyed.');return false;}
  // Distant spectrum analysis (FSS-style) before close survey.
  if(!spectrumed&&d<=p.r+st.spectrumRange){this.spectrumScan={id:p.id,progress:0};this.notify('Spectrum scanner locked. Hold for body analysis.');return true;}
  ...
  if(d>p.r+st.scanRange){this.auto=p;this.notify('Approach for detailed survey, or spectrum-scan from farther out.');return false;}
  this.scan={id:p.id,progress:0};this.notify('Survey array active. Hold position.');return true;}
