...
export function validateSave(x){
 ...
 if(x.missions.length>3||x.missions.some(m=>!m||!['delivery','survey','bounty','mining','escort','salvage','geology'].includes(m.type)||!SYSTEMS[m.origin]||!Number.isFinite(m.reward)||m.reward<0||!/^\d+-\d+$/.test(m.id)||typeof m.name!=='string'||(m.type==='delivery'&&(!SYSTEMS[m.destination]||!Number.isInteger(m.tons)||m.tons<1||m.tons>10))||(m.type==='survey'&&!SYSTEMS[m.destination])||((m.type==='bounty'||m.type==='escort')&&!Number.isFinite(m.startKills))||(m.type==='mining'&&m.tons!==5)||(m.type==='salvage'&&(!Number.isInteger(m.tons)||m.tons<1||m.tons>10))||(m.type==='geology'&&!Number.isFinite(m.startGeology))))return null;
...
export class Game {
 ...
 missionReady(m){return m.type==='delivery'?this.s.system===m.destination:this.s.system===m.origin&&(m.type==='survey'?surveyWorldIds(m.destination,SYSTEMS[m.destination]).every(id=>this.s.scanned.includes(id)):m.type==='bounty'||m.type==='escort'?(this.s.metrics?.pirates??this.s.kills)-m.startKills>=2:m.type==='geology'?(this.s.metrics?.geology||0)-(m.startGeology||0)>=(m.required||2):m.type==='salvage'?(this.s.cargo[m.good||'crystal']||0)>=(m.tons||3):this.s.cargo.ore>=m.tons);}
 claimMissions(){if(!this.s.docked)return;const done=this.s.missions.filter(m=>this.missionReady(m));for(const m of done){this.s.credits+=m.reward;this.s.contracts++;if(m.type==='mining')this.s.cargo.ore-=m.tons;if(m.type==='salvage')this.s.cargo[m.good||'crystal']-=m.tons;this.s.completed.push(m.id);this.notify(`Contract fulfilled · +${m.reward.toLocaleString()} cr`,'good');}this.s.missions=this.s.missions.filter(m=>!done.includes(m));}
