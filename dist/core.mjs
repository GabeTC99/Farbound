...
export function getStats(s){const b=SHIPS.find(x=>x.id===s.ship)||SHIPS[0],u=s.upgrades,m=moduleBonuses(s),fit=fittedWeaponDef(s),weapon=fit?.bonus?.weapon||b.weapon||'pulse',w=WEAPONS[weapon]||WEAPONS.pulse;return {...b,hull:b.hull+m.hull,damage:b.damage+u.laser*7+m.damage,shield:b.shield+u.shield*30+m.shield,speed:b.speed+u.engine*28+m.speed,range:b.range+u.drive*3+m.range,cargo:b.cargo+u.cargo*6+m.cargo,fuel:b.fuel+m.fuel,scanRange:650+m.scanRange,scanSpeed:m.scanSpeed,dataMultiplier:1+m.dataBonus,signalMultiplier:1+m.signalBonus,surfaceRange:m.surfaceRange,fuelEfficiency:Math.min(.5,m.fuelEfficiency),miningYield:1+m.miningYield,miningDamage:(b.miningDamage||20)+m.miningDamage,weapon,weaponName:fit?.name||w.name,weaponColor:w.color,energyMax:100,missileMax:weapon==='missile'?6+(b.class==='combat'?2:0):0};}
