import {moduleBonuses,expandGalaxy} from './catalog.mjs';
...
export const SHIPS = [
 ...
 {id:'rook',name:'Rook',class:'miner',role:'Prospecting cutter',price:6800,hull:140,shield:90,cargo:28,fuel:120,range:14,speed:200,turn:3.0,damage:22,slots:6,size:20,radius:18,color:'#d4b896',accent:'#b8956a',desc:'First claim on the rocks. Tough enough for belt work.'},
 ...
 {id:'jackal',name:'Jackal',class:'combat',role:'Patrol cutter',price:16500,hull:130,shield:140,cargo:14,fuel:125,range:14,speed:255,turn:4.3,damage:34,slots:6,size:19,radius:17,color:'#f0a898',accent:'#d47868',desc:'First dedicated fighter. Lean, mean, and short on patience.'},
 ...
 {id:'mole',name:'Mole',class:'miner',role:'Ore barge',price:20500,hull:200,shield:120,cargo:42,fuel:145,range:15,speed:175,turn:2.6,damage:24,slots:7,size:25,radius:23,color:'#c4a882',accent:'#a88860',desc:'A tanky hold for the long dig. Slow, stubborn, profitable.'},
 ...
 {id:'falcon',name:'Falcon',class:'combat',role:'Interceptor',price:25500,hull:135,shield:155,cargo:16,fuel:130,range:15,speed:310,turn:4.8,damage:36,slots:6,size:18,radius:16,color:'#ffb088',accent:'#e88858',desc:'Speed is the weapon. Catch them before they fold.'},
 ...
 {id:'vulture',name:'Vulture',class:'combat',role:'Privateer',price:36000,hull:175,shield:170,cargo:30,fuel:140,range:16,speed:260,turn:3.9,damage:40,slots:6,size:21,radius:19,color:'#d8a0b0',accent:'#b87890',desc:'Bite enough for pirates, hold enough for salvage.'},
 ...
 {id:'badger',name:'Badger',class:'miner',role:'Excavator',price:44000,hull:240,shield:150,cargo:50,fuel:155,range:16,speed:185,turn:2.7,damage:28,slots:8,size:26,radius:24,color:'#c8b078',accent:'#a89058',desc:'End-mid excavator. Digs deep and brings the seam home.'},
 {id:'raptor',name:'Raptor',class:'combat',role:'Gunship',price:48000,hull:190,shield:210,cargo:18,fuel:145,range:15,speed:270,turn:4.1,damage:48,slots:7,size:22,radius:20,color:'#f09888',accent:'#d07060',desc:'Dedicated firepower. The hangar that ends arguments.'},
 ...
 {id:'eagle',name:'Eagle',class:'combat',role:'Battlecruiser',price:78000,hull:260,shield:240,cargo:22,fuel:165,range:17,speed:245,turn:3.5,damage:55,slots:8,size:26,radius:24,color:'#ff9080',accent:'#e06858',desc:'Endgame warship. Shields like a station, teeth to match.'}
];
...
export function getStats(s){const b=SHIPS.find(x=>x.id===s.ship)||SHIPS[0],u=s.upgrades,m=moduleBonuses(s);return {...b,hull:b.hull+m.hull,damage:b.damage+u.laser*7+m.damage,shield:b.shield+u.shield*30+m.shield,speed:b.speed+u.engine*28+m.speed,range:b.range+u.drive*3+m.range,cargo:b.cargo+u.cargo*6+m.cargo,fuel:b.fuel+m.fuel,scanRange:650+m.scanRange,scanSpeed:m.scanSpeed,dataMultiplier:1+m.dataBonus,signalMultiplier:1+m.signalBonus,surfaceRange:m.surfaceRange,fuelEfficiency:Math.min(.5,m.fuelEfficiency),miningYield:1+m.miningYield};}
...
export function validateSave(x){
 ...
 if(x.visited.some(i=>!Number.isInteger(i)||!SYSTEMS[i])||x.scanned.some(id=>!/^planet-\d+-\d+$/.test(id)))return null;
...
export class Game {
 ...
 makeSystem(){
  ...
  this.planets=layout.planets;this.belts=layout.belts;this.belt=layout.belts[0]||null;
 ...
 scanTarget(){if(this.scan||this.jump)return false;if(Math.hypot(this.player.vx,this.player.vy)>100){this.notify('Slow below 100 m/s to survey a world.');return false;}if(this.s.docked){this.notify('Launch and approach a world to scan it.');return false;}const p=this.target?.type==='planet'?this.target:this.planets.reduce((a,b)=>dist(this.player,a)<dist(this.player,b)?a:b);this.target=p;if(this.s.scanned.includes(p.id)){this.notify('This world is already surveyed.');return false;}if(dist(this.player,p)>p.r+getStats(this.s).scanRange){this.auto=p;this.notify('Approaching scan range. Tap SCAN when you arrive.');return false;}this.scan={id:p.id,progress:0};this.notify('Survey array active. Hold position.');return true;}
 ...
 update(dt,input={}){
  ...
  if(this.scan){const t=this.planets.find(p=>p.id===this.scan.id);if(!t||dist(p,t)>t.r+st.scanRange+50||Math.hypot(p.vx,p.vy)>100||input.fire){this.scan=null;this.notify('Scan interrupted. Stay within range, below 100 m/s, and hold fire.');}else{this.scan.progress+=dt*(1+st.scanSpeed);if(this.scan.progress>=3){this.s.scanned.push(t.id);const surveyValue=Math.round(t.value*st.dataMultiplier);this.s.data+=surveyValue;this.effects.push({x:t.x,y:t.y,r:t.r,color:'#84f3db',life:2});this.scan=null;this.s.tutorial=Math.max(this.s.tutorial,2);this.notify(`Survey complete · ${surveyValue.toLocaleString()} cr in data. Sell by docking.`,'good');}}}
