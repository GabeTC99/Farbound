...
import {Game,newSave,validateSave,SYSTEMS,SHIPS,GOODS,UPGRADES,getStats,cargoUsed,price,contractsFor,jumpDistance,jumpCost,dist,clamp,rng,systemSky,wantedTier,systemName} from './frontier.mjs';
import {FACTIONS,missionDestination,operationDetails,nearestAnomaly,terrainAt} from './frontier.mjs';
...
const leaveStation=()=>{if(game.s.docked){if(!game.launch()){stationTab=game.sys.prison||game.s.detained?'detention':'market';openPanel('station');return;}}closePanel();};
...
function updateHUD(){
 ...
 $('flight-status').textContent=s.docked?(s.detained?'DETENTION HOLD':game.sys.prison?'PRISON BARGE':'DOCKING BAY 04'):game.auto?'AUTOPILOT ENGAGED':game.jump?'FOLD TRANSIT':'LOCAL SPACE';$('flight-location').textContent=s.docked?sys.station:sys.name+' system';$('flight-desc').textContent=s.docked?(s.detained?'Pay the detention fine to clear your standing and launch.':game.sys.prison?'Detention services, limited market access, and release processing.':'Trade, refuel, and choose your next destination.'):sys.lore;
 $('target-name').textContent=t?.name||'No target';$('target-type').textContent=t?.type==='planet'?(s.scanned.includes(t.id)?'Surveyed':'Unsurveyed world'):t?.type==='enemy'?'Wanted vessel':t?.type==='wake'?(t.scanned?'Resolved hyperspace wake':'Unresolved hyperspace wake'):t?.type==='traffic'?((t.hull?t.hull[0].toUpperCase()+t.hull.slice(1)+' · ':'')+(t.status||'Civilian vessel')):t?.type==='station'?'Orbital station':t?.type==='belt'?'Mineral deposits':'Resource';$('target-dist').textContent=d>1000?(d/1000).toFixed(1)+' km':Math.round(d)+' m';
 const context=$('context-action');context.textContent=s.docked?'STATION SERVICES':t?.type==='wake'?(t.scanned?'PLOT WAKE ROUTE':d<=260?'SCAN WAKE':'APPROACH WAKE'):t?.type==='station'?(d<=200?'DOCK':'APPROACH'):t?.type==='planet'?(s.scanned.includes(t.id)?'SURVEYED':d<t.r+st.scanRange?'SCAN WORLD':'APPROACH'):t?.type==='enemy'?'INTERCEPT':'APPROACH';context.disabled=!s.docked&&t?.type==='planet'&&s.scanned.includes(t.id);
...
function updateFrontierHUD(){
 ...
 const cataloged=s.systemScans.includes(s.system),discovery=$('discovery-button');discovery.hidden=!!surface;discovery.disabled=s.docked||!!game.jump||!!game.scan||!!game.discoveryScan||cataloged;discovery.textContent=game.discoveryScan?'PULSE CHARGING…':cataloged?'SYSTEM CATALOGED':'DISCOVERY PULSE';
 $('scanner-status').hidden=!!surface;$('scanner-status').textContent=(cataloged?'Catalog complete':game.sys.uncharted?'Discovery pulse locates worlds':'Known stellar charts')+' · '+game.visiblePlanets.length+' worlds located · '+game.planets.filter(p=>s.scanned.includes(p.id)).length+'/2 surveyed';
 ...
 const activeOp=s.operations[0];if(activeOp&&!game.surface)$('flight-desc').textContent=operationDetails(s,activeOp).next;
 ...
 }else if(t?.type==='star'){$('target-type').textContent='Primary star · fuel source';$('context-action').textContent=game.scooping?'STOP SCOOPING':s.fuel>=st.fuel?'TANK FULL':dist(game.player,game.star)>game.star.r+600?'APPROACH TO SCOOP':'SCOOP FUEL';$('context-action').disabled=s.docked||!!game.jump||(!game.scooping&&s.fuel>=st.fuel);}
 if(!surface){
 ...
 if(!surface&&t?.faction){$('target-type').textContent=FACTIONS.find(f=>f.id===t.faction)?.name||'Faction vessel';$('context-action').textContent=t.type==='faction'?'ENGAGE PATROL':'INTERCEPT';}
}
...
function renderPanel(){
 ...
 if(panel==='menu'||panel==='help'){
  html=modalShell(panel==='help'?'Your first flight':'Flight menu',`FARBOUND · v${RELEASE}${simPaused()?' · PAUSED':''}`,`${panel==='help'?helpHTML():`<div class="setting-row"><div><h3>Flight manual</h3><p>Controls, earning credits, and staying in flight.</p></div><button data-action="help">Open manual</button></div><div class="setting-row"><div><h3>Sound effects</h3><p>Engine and interface sounds. Set engine volume independently below.</p></div><button class="${s.sound?'switch-on':''}" data-action="sound">${s.sound?'Sound on':'Sound off'}</button></div><div class="setting-row"><div><h3>Engine hum</h3><p>Engine volume from silent to full. Pauses in menus.</p></div><label class="volume-control"><span id="volume-value">${Math.round(s.engineVolume*100)}%</span><input id="engine-volume" aria-label="Engine volume" type="range" min="0" max="100" value="${Math.round(s.engineVolume*100)}"></label></div><div class="setting-row"><div><h3>Install on Android</h3><p>${offlineReady?'Offline game files are ready.':'Open online once to prepare offline play.'} In Chrome, use ⋮ → Add to Home screen → Install.</p></div><button data-action="install">Install game</button></div><div class="setting-row"><div><h3>Save progress</h3><p>${storageOK?'Auto-saved on this device.':'Device storage unavailable.'} Export a backup to move your pilot to another device.</p></div><button data-action="export">Export save</button></div><div class="setting-row"><div><h3>Restore a pilot</h3><p>Import a Farbound save file. Replaces this device’s current progress.</p></div><label><span class="sr-only">Import save file</span><input class="file-input" id="import-save" type="file" accept=".json,application/json"></label></div><div class="setting-row"><div><h3>Roll back progress</h3><p>A rolling checkpoint keeps a recent copy of this pilot. Export a save for a permanent backup.</p></div><button data-action="restore-checkpoint">Restore checkpoint</button></div><div class="setting-row"><div><h3>Original game · v1.0</h3><p>Open the original release with your original pilot. Frontiers progress remains saved separately.</p></div><a class="button-link" href="./classic/index.html">Play original v1</a></div><div class="setting-row"><div><h3>Emergency recovery</h3><p>Return to the station with a full tank. Costs up to 250 cr; cargo, delivery contracts and unsold data and anomaly signals are lost.</p></div><button class="danger" data-action="rescue-prompt">Call recovery</button></div><div class="setting-row"><div><h3>New journey</h3><p>Start fresh after preserving a checkpoint of this pilot.</p></div><button class="danger" data-action="reset-prompt">Start over</button></div><div class="setting-row"><div><h3>Temporary DEV menu</h3><p>Teleport, credits, bounty, wakes, and response teams for testing.</p></div><button data-action="dev">Open DEV tools</button></div><p class="detail-text" style="margin-top:18px">Release ${RELEASE_NAME} ${RELEASE}. Station services keep local space running; other menus pause flight.</p>`}`);
 }
 if(panel==='dev'){
...
function contractsView(atStation){const s=game.s;let out='';if(s.missions.length){out+='<h3 style="margin-bottom:15px">Active · '+s.missions.length+' / 3</h3><div class="cards">'+s.missions.map(m=>{let prog=m.type==='delivery'?'Deliver to '+SYSTEMS[m.destination].name:m.type==='survey'?[0,1].filter(n=>s.scanned.includes(`planet-${m.destination}-${n}`)).length+' / 2 worlds scanned':m.type==='bounty'?Math.min(2,s.metrics.pirates-m.startKills)+' / 2 pirates defeated':s.cargo.ore+' / 5 t titanium aboard';return `<article class="card"><span class="tag">${m.type}</span><h3>${esc(m.name)}</h3><p>${esc(m.desc||'Complete the contract objectives.')}</p><p class="accent">${prog}</p><div class="contract-route"><button data-action="route-contract" data-id="${m.id}">Plot destination route ↗</button><span>${jumpDistance(s.system,missionDestination(s,m)).toFixed(1)} ly away</span></div><div class="card-bottom"><b>${fmt(m.reward)} cr</b><button class="quiet" data-action="abandon-prompt" data-id="${m.id}">Abandon</button></div><div class="detail-text" style="margin-top:10px">${m.type==='delivery'?'Paid on delivery.':'Return to '+SYSTEMS[m.origin].station+' in '+SYSTEMS[m.origin].name+'.'}</div></article>`;}).join('')+'</div>';}
 ...
 else if(s.docked)out+='<button style="margin-top:20px" data-action="station-contracts">Open station board</button>';else out+='<p class="intro" style="margin-top:20px">Completed contracts pay automatically when you dock at their destination. Station services keep local space running; other menus pause flight.</p>';
...
async function action(a,id){
 ...
 switch(a){
  ...
  case 'close':if(panel==='station'&&game.s.docked)leaveStation();else closePanel();break;
  ...
  case 'launch':if(game.launch())closePanel();else{stationTab=game.sys.prison||game.s.detained?'detention':'market';renderPanel();}break;
...
document.addEventListener('keydown',e=>{
 ...
 if(e.key==='Escape'){if(panel==='station'&&game.s.docked)leaveStation();else if(panel)closePanel();else if(started)openPanel('menu');return;}if(simPaused()||!started)return;
 const k=e.key.toLowerCase();keys[k]=true;if(k==='n')openPanel('map');if(k==='e'&&!game.surface){if(game.dock())openPanel('station');}if(k==='r')game.scanTarget();if(k==='h')game.discover();if(k==='p'&&!game.surface)game.autopilot();if(k==='l')action('land-or-takeoff');if(k===' '&&game.surface)game.scanSurface();
...
canvas.addEventListener('pointerdown',e=>{if(panel||!started||game.s.docked||game.surface)return;const p={x:(e.clientX-width/2)/cam.zoom+cam.x,y:(e.clientY-height/2)/cam.zoom+cam.y};const all=[...(game.sys.hasStation?[game.station]:[]),game.star,...game.wakes,...game.patrols,...game.traffic,...game.visiblePlanets,...game.enemies,...game.asteroids];const target=all.filter(t=>dist(p,t)<Math.max(t.r,35/cam.zoom)).sort((a,b)=>dist(p,a)-dist(p,b))[0];if(target){game.target=target;game.auto=null;updateHUD();}});
...
function render(dt=1/60){
 ...
 const orbitColor='#2c52652b';ctx.lineWidth=1;ctx.setLineDash([2,14]);for(const p of game.visiblePlanets)circle(0,0,dist(p,game.station),orbitColor,true);ctx.setLineDash([]);
 for(const p of game.visiblePlanets)drawPlanet(p);if(game.sys.hasStation)drawStation();drawStar();for(const p of game.patrols){drawSecurityShip(p);label(p.status==='ENGAGING'?'SECURITY ENGAGING':p.status==='SEARCHING'?'SECURITY SEARCHING':p.status==='RESPONDING'?'SECURITY RESPONDING':p.name,p.x,p.y-38,'#9eb7bd',12);}
