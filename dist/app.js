...
let keys={},touch={aim:null,thrust:0,fire:false,boost:false},stickPointer=null,padActive=false,padHintShown=false,showTutorial=game.s.tutorial<2,lastStore=0,lastHUD=0,clock=0,saveErrorShown=false,confirmAction=null,lastFocus=null,shipBank=0,playerTrail=[];
...
function shiftPanelFocus(dir){const items=panelFocusables();if(!items.length)return;const i=items.indexOf(document.activeElement);let n;if(i<0)n=dir>0?0:items.length-1;else n=(i+dir+items.length)%items.length;items[n].focus();}
function handlePadEdges(edges){
 for(const edge of edges){
  ...
  if(edge==='confirm'){
   ...
   if(el&&$('panel-layer')?.contains(el)&&(el.matches('button,a[href]')||el.type==='submit')){el.click();continue;}
   if(panel==='confirm'){action('confirm');continue;}
   $('panel-layer')?.querySelector('button.primary,button:not(.quiet):not(.icon-button)')?.click();
   continue;
  }
  if(edge==='cancel'){
   if(!panel)continue;
  ...
  if(edge==='focus-prev'){if(panel)shiftPanelFocus(-1);continue;}
  if(edge==='focus-next'){if(panel)shiftPanelFocus(1);continue;}
  ...
  else if(edge==='autopilot')action('autopilot');
  else if(edge==='land')action('land-or-takeoff');
 }
}
function buildHUD(){
...
function updateHUD(){
 ...
 const context=$('context-action');context.textContent=s.docked?'INTERACT':t?.type==='wake'?(t.scanned?'PLOT WAKE ROUTE':d<=260?'SCAN WAKE':'APPROACH WAKE'):t?.type==='signal'?(t.scanned?'RESOLVED':d<=280?'SCAN SIGNAL':'APPROACH'):t?.type==='derelict'?(t.salvaged?'SALVAGED':t.scanned?(d<=180?'BOARD WRECK':d<=200?'SALVAGE':'APPROACH'):d<=280?'SCAN SIGNAL':'APPROACH'):t?.type==='station'?(d<=200?'DOCK':'APPROACH'):isBody?(surveyed?'SURVEYED':d<t.r+st.scanRange&&spectrumed?'SCAN WORLD':d<t.r+st.spectrumRange?(spectrumed?'APPROACH SURVEY':'SPECTRUM SCAN'):'APPROACH'):t?.type==='enemy'?'INTERCEPT':'APPROACH';context.disabled=!s.docked&&((isBody&&surveyed)||(t?.type==='signal'&&t.scanned)||(t?.type==='derelict'&&t.salvaged));
...
function updateFrontierHUD(){
 ...
 if(scanBtn&&!surface&&!onfoot){
  ...
  scanBtn.textContent=game.spectrumScan?'ANALYZING…':game.scan?'SURVEYING…':(body&&!spect&&d<body.r+st.spectrumRange)?'SPECTRUM':(body&&spect&&!s.scanned.includes(body.id))?'SURVEY':'SCAN';
 }
