...
function updateHUD(){
 ...
 const context=$('context-action');context.textContent=s.docked?'INTERACT':t?.type==='wake'?(t.scanned?'PLOT WAKE ROUTE':d<=260?'SCAN WAKE':'APPROACH WAKE'):t?.type==='signal'||t?.type==='derelict'?(t.scanned?'RESOLVED':d<=280?'SCAN SIGNAL':'APPROACH'):t?.type==='station'?(d<=200?'DOCK':'APPROACH'):isBody?(surveyed?'SURVEYED':d<t.r+st.scanRange&&spectrumed?'SCAN WORLD':d<t.r+st.spectrumRange?(spectrumed?'APPROACH SURVEY':'SPECTRUM SCAN'):'APPROACH'):t?.type==='enemy'?'INTERCEPT':'APPROACH';context.disabled=!s.docked&&((isBody&&surveyed)||((t?.type==='signal'||t?.type==='derelict')&&t.scanned));
...
function updateFrontierHUD(){
 ...
 if(onfoot&&surface){
  ...
  $('flight-status').textContent=surface.landed?'LANDED':'SURFACE EXPEDITION';$('flight-location').textContent=surface.planetName;$('flight-desc').textContent=surface.landed?'Disembark to inspect nearby signals on foot, or return to orbit.':'Fly the skiff. Soft-land, then disembark. Hover-scan still works in the air.';
  $('shield-label').textContent='SKIFF '+Math.ceil(surface.integrity)+'%';$('shield-meter').style.width=surface.integrity+'%';
...
function contractsView(atStation){
 ...
 if(s.missions.length){
  out+='<h3 style="margin-bottom:15px">Active · '+s.missions.length+' / 3</h3><div class="cards">'+s.missions.map(m=>{
   ...
   let prog=m.type==='delivery'?'Deliver to '+SYSTEMS[m.destination].name:m.type==='survey'?worlds.filter(id=>s.scanned.includes(id)).length+' / '+worlds.length+' worlds scanned':m.type==='bounty'?Math.min(2,s.metrics.pirates-m.startKills)+' / 2 pirates defeated':s.cargo.ore+' / '+(m.tons||5)+' t titanium aboard';
