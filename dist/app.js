...
async function cloudBoot(){if(!cloudConfigured())return;try{consumeAuthRedirect();const session=await ensureCloudSession();if(session){cloudMeta=await fetchCloudPilotMeta().catch(()=>null);cloudStatus=`Signed in as ${cloudUserEmail()}.`;}}catch(err){cloudStatus=err.message||'Cloud session error.';}}
cloudBoot();
...
async function action(a,id){
 ...
 switch(a){
  ...
  try{await requestCloudSignIn(cloudEmailDraft);cloudStatus='Check your email for a sign-in link (and code, if enabled). Keep this tab open or return here after opening the link.';game.notify('Sign-in email sent.','good');}
