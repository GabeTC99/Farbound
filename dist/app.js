...
<div class="setting-row"><div><h3>FPS meter</h3><p>Live requestAnimationFrame rate from the game loop. Off by default. Samsung Game Booster reads 0 for this PWA.</p></div><button class="${s.showFps?'switch-on':''}" data-action="dev-fps">${s.showFps?'FPS on':'Show FPS'}</button></div>
...
function paintFpsOverlay(){
 const el=$('fps-meter');
 if(!el)return;
 if(game.s.showFps!==true){if(!el.hidden)el.hidden=true;return;}
 ...
 if(det)det.textContent=snap.n?snap.ms.toFixed(1)+' ms · 1% '+Math.round(snap.low1):'sampling…';
}
function loop(now){
