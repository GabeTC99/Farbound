...
import {ensureCloudSession,requestCloudSignIn,verifyCloudOtp,cloudSignOut,cloudUserEmail,uploadCloudPilot,downloadCloudPilot,fetchCloudPilotMeta,cloudAutosyncEnabled,setCloudAutosync,consumeAuthRedirect,comparePilotFreshness} from './cloud-sync.mjs';
...
let cloudBusy=false,cloudEmailDraft='',cloudOtpDraft='',cloudStatus='',cloudMeta=null;
...
function queueCloudAutosync(){clearTimeout(cloudAutosyncTimer);cloudAutosyncTimer=setTimeout(async()=>{if(cloudBusy||!cloudAutosyncEnabled()||!cloudUserEmail()||!game.s.docked)return;cloudBusy=true;try{const data=game.serialize();data.savedAt=Date.now();const row=await uploadCloudPilot(data);cloudMeta={updated_at:row.updated_at,playtime:row.playtime,credits:row.credits,system:row.system,ship:row.ship};try{localStorage.setItem('farbound-cloud-pushed-at',row.updated_at||new Date().toISOString());}catch{};}catch(err){console.warn('[cloud]',err.message||err);}cloudBusy=false;},2500);}
...
function updateFrontierHUD(){
 ...
 if(onfoot&&surface){
  ...
  $('speed').textContent=Math.round(Math.hypot(surface.vx,surface.vy));$('motion-mode').textContent=surface.landed?'LANDED':(Math.round(alt)+' m · SINK '+sink);$('boost')?.classList.toggle('held',!!(touch.boost||keys.shift)&&!panel);
...
async function action(a,id){
 ...
 switch(a){
  ...
  case 'cloud-send-link':{
  cloudEmailDraft=($('cloud-email')?.value||cloudEmailDraft||'').trim();
  cloudBusy=true;cloudStatus='Sending sign-in email…';renderPanel();
  try{await requestCloudSignIn(cloudEmailDraft);cloudStatus='Email sent. Open the sign-in link in Chrome (not inside Gmail). It should return to Farbound signed in. Keep this game tab open.';game.notify('Sign-in email sent.','good');}
  catch(err){cloudStatus=err.message||'Sign-in failed.';game.notify(cloudStatus,'bad');}
  cloudBusy=false;renderPanel();break;}
 case 'cloud-verify-otp':{
  cloudEmailDraft=($('cloud-email')?.value||cloudEmailDraft||'').trim();
  cloudOtpDraft=($('cloud-otp')?.value||cloudOtpDraft||'').trim();
  cloudBusy=true;cloudStatus='Verifying code…';renderPanel();
  try{await verifyCloudOtp(cloudEmailDraft,cloudOtpDraft);cloudMeta=await fetchCloudPilotMeta();cloudStatus=`Signed in as ${cloudUserEmail()}.`;game.notify('Cloud account linked.','good');}
  catch(err){cloudStatus=err.message||'Verification failed.';game.notify(cloudStatus,'bad');}
  cloudBusy=false;renderPanel();break;}
 ...
 case 'cloud-upload':{
  ...
  try{save();const row=await uploadCloudPilot(game.serialize());cloudMeta={updated_at:row.updated_at,playtime:row.playtime,credits:row.credits,system:row.system,ship:row.ship};try{localStorage.setItem('farbound-cloud-pushed-at',row.updated_at||new Date().toISOString());}catch{}cloudStatus=`Uploaded ${new Date(cloudMeta.updated_at).toLocaleString()}.`;game.notify('Pilot uploaded to the cloud.','good');}
...
function loop(now){const dt=Math.min(.05,(now-last)/1000);last=now;if(!document.hidden){clock+=dt;if(started&&!simPaused()){const wasDocked=game.s.docked;const wasFolding=!!game.jump;const canWalk=!panel;const input={aim:canWalk?touch.aim:null,thrust:canWalk?Math.max(touch.thrust,keys.w||keys.arrowup?1:0):0,turn:canWalk?(keys.d||keys.arrowright?1:0)-(keys.a||keys.arrowleft?1:0):0,fire:!panel&&(touch.fire||keys[' ']),boost:canWalk&&(touch.boost||keys.shift),brake:canWalk&&(keys.s||keys.arrowdown)};game.update(dt,input);if(input.fire&&!game.surface&&!game.onfoot&&!game.s.docked&&now-lastFireSound>240){sound('fire');lastFireSound=now;}if(!wasFolding&&game.jump){try{engine.unlock();}catch{}}if(wasFolding&&!game.jump&&game.s.sound){const vol=Number.isFinite(game.s.engineVolume)?game.s.engineVolume:.35;try{engine.playFoldJump(vol);engine.playFoldArrive(vol);}catch{}}if(!wasDocked&&game.s.docked){try{engine.unlock();}catch{}}else if(wasDocked&&!game.s.docked)closePanel();}render(dt);if(panel==='system-map')drawSystemMap();const onDeck=!!(game.onfoot&&game.s.docked);const planetFeet=!!(game.onfoot&&game.surface);const p=game.onfoot||game.surface||game.player;const ambience=Number.isFinite(game.s.engineVolume)?game.s.engineVolume:.35;const sky=systemSky(game.sys).kind;engine.update({moving:Math.min(1,Math.hypot(p.vx||0,p.vy||0)/(game.surface&&!game.onfoot?140:onDeck||planetFeet?280:getStats(game.s).speed)),boost:game.boost||touch.boost,volume:ambience,enabled:game.s.sound,paused:!started||(simPaused()&&!onDeck&&!planetFeet),surface:!!game.surface&&!game.onfoot,station:onDeck,planetFeet,sky,surfaceKind:game.surface?.kindId||'mineral'});try{engine.setFoldCharge(started&&game.jump&&!simPaused()?game.jump.progress/3:0,ambience,!!game.s.sound&&started&&!document.hidden);}catch{}if(now-lastHUD>120){updateHUD();drainEvents();lastHUD=now;}if(started&&now-lastStore>4000){save();lastStore=now;}}else{engine.muteAll();}requestAnimationFrame(loop);}
