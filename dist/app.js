...
<div class="setting-row"><div><h3>FPS meter</h3><p>Live requestAnimationFrame rate from the game loop (not Game Booster). A compositor keep-alive runs during play so Chrome/Samsung are more likely to stay at 120 without touch. If hands-off cruise still pins 60 after ~2s, that is OS Adaptive / Game Optimizer — see Flight menu → Graphics. Off by default.</p></div><button class="${s.showFps?'switch-on':''}" data-action="dev-fps">${s.showFps?'FPS on':'Show FPS'}</button></div>
