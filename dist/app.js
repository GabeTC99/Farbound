...
let liteMode=false;function refreshLite(){liteMode=matchMedia('(max-width:700px), (pointer:coarse)').matches;}refreshLite();
try{matchMedia('(max-width:700px)').addEventListener('change',refreshLite);matchMedia('(pointer:coarse)').addEventListener('change',refreshLite);}catch{}
const liteFX=()=>liteMode;
...
function updateFrontierHUD(){
 ...
 const cataloged=s.systemScans.includes(s.system),discovery=$('discovery-button');discovery.hidden=!!surface||!!onfoot;discovery.disabled=s.docked||!!game.jump||!!game.scan||!!game.discoveryScan||cataloged; const compactUI=liteFX();discovery.textContent=game.discoveryScan?(compactUI?'CHARGING…':'PULSE CHARGING…'):cataloged?(compactUI?'CATALOGED':'SYSTEM CATALOGED'):(compactUI?'PULSE':'DISCOVERY PULSE');
 $('scanner-status').hidden=!!surface||!!onfoot||compactUI;$('scanner-status').textContent=(cataloged?'Catalog complete':game.sys.uncharted?'Discovery pulse locates worlds':'Known stellar charts')+' · '+game.visiblePlanets.length+' worlds located · '+game.planets.filter(p=>s.scanned.includes(p.id)).length+'/'+game.planets.length+' surveyed';
 ...
 $('landing-button').hidden=!!(onfoot&&surface)||(!surface&&(s.docked||t?.type!=='planet'||(t&&t.landable===false)));$('landing-button').textContent=surface?'RETURN TO ORBIT':'LAND ON WORLD';
...
function resize(){refreshLite();width=innerWidth;height=innerHeight;dpr=canvasDpr();canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);if(panel==='map')drawMap();}window.addEventListener('resize',resize);
...
function drawShip(x,y,a,color,size=19,enemy=false,thrust=0,boost=false){ctx.save();ctx.translate(x,y);ctx.rotate(a);if(thrust>.05){const len=(liteFX()?12:(12+Math.random()*(boost?47:18)))*thrust;ctx.fillStyle='#76efdb';ctx.globalAlpha=.7;ctx.beginPath();ctx.moveTo(-size+3,-5);ctx.lineTo(-size-len,0);ctx.lineTo(-size+3,5);ctx.fill();ctx.globalAlpha=1;}ctx.fillStyle=enemy?'#4d252d':'#183742';ctx.strokeStyle=color;ctx.lineWidth=1.7;ctx.beginPath();ctx.moveTo(size*1.25,0);ctx.lineTo(-size,size*.74);ctx.lineTo(-size*.55,0);ctx.lineTo(-size,-size*.74);ctx.closePath();ctx.fill();ctx.stroke();ctx.strokeStyle=color;ctx.globalAlpha=.7;ctx.beginPath();ctx.moveTo(size*.85,0);ctx.lineTo(-size*.5,0);ctx.stroke();ctx.fillStyle='#c8f8f2';ctx.fillRect(0,-2,5,4);ctx.restore();}
...
function drawPlayerShip(dt=1/60){
 ...
 if(thrust>.08&&!liteFX()){playerTrail.push({x:p.x-Math.cos(p.angle)*(size-2),y:p.y-Math.sin(p.angle)*(size-2),life:.4,color:boost?hull.accent:color});if(playerTrail.length>18)playerTrail.shift();}else if(liteFX())playerTrail.length=0;
 if(!liteFX()){for(const t of playerTrail){t.life-=dt;ctx.globalAlpha=Math.max(0,t.life*1.8);circle(t.x,t.y,boost?2.4:1.7,t.color);}ctx.globalAlpha=1;playerTrail=playerTrail.filter(t=>t.life>0);}
...
function drawTrafficShip(tr){
 const size=tr.size||12,color=tr.color||'#647d94',lite=liteFX();
...
function drawPlanet(p){
 ...
 if(liteFX()){circle(p.x,p.y,p.r,p.color);ctx.lineWidth=1;circle(p.x,p.y,p.r,p.color+'88',true);}
 else{
 ...
 label(p.name,p.x,p.y+p.r+34,'#adbec7');if(!liteFX()){if(p.kind)label(p.kind,p.x,p.y+p.r+52,'#718694',11);label(game.s.scanned.includes(p.id)?'SURVEYED':'UNSURVEYED',p.x,p.y+p.r+(p.kind?70:58),game.s.scanned.includes(p.id)?'#83d9c9':'#718694',12);}
}
...
function render(dt=1/60){
 ...
 if(!liteFX()){const orbitColor='#2c52652b';ctx.lineWidth=1;ctx.setLineDash([2,14]);for(const star of (game.stars||[game.star]))for(const p of game.visiblePlanets)circle(star.x,star.y,dist(p,star),orbitColor,true);ctx.setLineDash([]);}
 ...
 {const lite=liteFX(),viewPad=(Math.hypot(width,height)*.5)/cam.zoom+80;for(const a of game.asteroids){if(lite&&Math.hypot(a.x-cam.x,a.y-cam.y)>viewPad)continue;if(lite){circle(a.x,a.y,a.r,a.ore==='crystal'?'#1e3f42':'#34343b');continue;}ctx.save();ctx.translate(a.x,a.y);ctx.rotate(a.rotation+clock*.025);ctx.fillStyle=a.ore==='crystal'?'#1e3f42':'#34343b';ctx.strokeStyle=a.ore==='crystal'?'#75b9b0':'#7b7874';ctx.lineWidth=1;ctx.beginPath();for(let i=0;i<9;i++){const ang=i/9*6.28,r=a.shape[i]*a.r;if(i===0)ctx.moveTo(Math.cos(ang)*r,Math.sin(ang)*r);else ctx.lineTo(Math.cos(ang)*r,Math.sin(ang)*r);}ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();}}
 if(!liteFX())for(const b of (game.belts||(game.belt?[game.belt]:[])))label(b.name.toUpperCase(),b.x,b.y-Math.max(180,b.r+40),'#829496',13);
 ...
 const softFX=!liteFX();for(const b of game.shots){ctx.strokeStyle=b.enemy?'#ff8b7b':(b.color||'#a4fff0');if(softFX){ctx.shadowBlur=b.kind==='beam'?14:9;ctx.shadowColor=ctx.strokeStyle;}ctx.lineWidth=b.enemy?3:b.kind==='missile'?4.5:b.kind==='beam'?2:3.5;ctx.beginPath();ctx.moveTo(b.x,b.y);ctx.lineTo(b.x-b.vx*(b.kind==='beam'?.01:.025),b.y-b.vy*(b.kind==='beam'?.01:.025));ctx.stroke();}if(softFX)ctx.shadowBlur=0;if(game.lock){const L=game.lock;ctx.strokeStyle='#a4fff0';ctx.globalAlpha=.85;ctx.lineWidth=1.5;ctx.strokeRect(L.x-18,L.y-18,36,36);ctx.globalAlpha=1;label('LOCK',L.x,L.y-28,'#a4fff0',11);}for(const h of (game.hitMarks||[])){ctx.globalAlpha=Math.max(0,h.life/.55);label(h.text,h.x,h.y-(.55-h.life)*20,h.color||'#a4fff0',12);ctx.globalAlpha=1;}if(game.threat>0){ctx.save();ctx.strokeStyle='rgba(239,139,127,'+Math.min(1,game.threat)+')';ctx.lineWidth=3;ctx.strokeRect(cam.x-width/(2*cam.zoom)+8,cam.y-height/(2*cam.zoom)+8,width/cam.zoom-16,height/cam.zoom-16);ctx.restore();}
 if(liteFX()){for(const p of game.particles){ctx.globalAlpha=Math.min(1,p.life*2);ctx.fillStyle=p.color;ctx.fillRect(p.x-1.5,p.y-1.5,3,3);}ctx.globalAlpha=1;}else{for(const p of game.particles){ctx.globalAlpha=Math.min(1,p.life*2);circle(p.x,p.y,2.2,p.color);}ctx.globalAlpha=1;}
