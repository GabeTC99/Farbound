...
function updateFrontierHUD(){
 ...
 $('scanner-status').hidden=!!surface;$('scanner-status').textContent=(cataloged?'Catalog complete':game.sys.uncharted?'Discovery pulse locates worlds':'Known stellar charts')+' · '+game.visiblePlanets.length+' worlds located · '+game.planets.filter(p=>s.scanned.includes(p.id)).length+'/'+game.planets.length+' surveyed';
...
function contractsView(atStation){
 ...
 else out+='<p class="intro" style="margin-top:20px">Completed contracts pay automatically when you dock at their destination. Station services keep local space running; other menus pause flight.</p>';
