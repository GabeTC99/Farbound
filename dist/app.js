...
function drawStar(p=game.star){const col=p.color||'#e9be82',g=ctx.createRadialGradient(p.x,p.y,p.r*.5,p.x,p.y,p.r*2.2);g.addColorStop(0,col+'aa');g.addColorStop(.45,col+'44');g.addColorStop(1,col+'00');circle(p.x,p.y,p.r*2.2,g);circle(p.x,p.y,p.r,col);circle(p.x-p.r*.13,p.y-p.r*.1,p.r*.74,'#f8d9a3');label(p.name,p.x,p.y+p.r+45,'#e8c18a');if(p.primary!==false&&p.id==='star'){circle(p.x,p.y,p.r+600,'#ecc08533',true);circle(p.x,p.y,p.r+150,'#ff777722',true);label('FUEL SCOOPING ZONE · WATCH HEAT',p.x,p.y+p.r+70,'#987c62',12);}else circle(p.x,p.y,p.r+320,col+'22',true);}
function drawWake(w){
