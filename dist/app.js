...
import {Game,newSave,validateSave,SYSTEMS,SHIPS,GOODS,UPGRADES,getStats,cargoUsed,price,contractsFor,jumpDistance,jumpCost,dist,clamp,rng,systemSky,wantedTier,systemName,EVENT_IDS,eventArrowTargets} from './frontier.mjs';
...
function renderPanel(){
 ...
 if(panel==='station'){
  ...
  const tabList=prison||detained?[['detention','Detention'],['market','Commissary'],['data','Cartographics'],['concierge','Concierge'],['outfitting','Modules'],['shipyard','Hangar']]:[['market','Market'],['data','Cartographics'],['contracts','Contracts'],['concierge','Concierge'],['outfitting','Modules'],['shipyard','Hangar'],['guilds','Guilds'],['factions','Factions']];
  ...
  if(stationTab==='concierge')body=robotView(game);
...
<div class="setting-row"><div><h3>${esc(STATION_ROBOT.displayName)} · Concierge</h3><p>Open the station robot desk or force a dialogue line.</p></div><div class="section-actions"><button data-action="dev-concierge">Open desk</button><button data-action="dev-robot-talk">Force line</button></div></div>
...
async function action(a,id){
 ...
 switch(a){
  ...
  case 'dev-concierge':game.teleportTo(0,{docked:true});game.s.detained=false;stationTab='concierge';panel='station';renderPanel();save();break;
  case 'dev-robot-talk':{if(!game.s.docked)game.teleportTo(game.s.system,{docked:true});const line=game.talkRobot();if(line)game.notify(STATION_ROBOT.displayName+' · '+line.text,'good');stationTab='concierge';panel='station';renderPanel();save();break;}
...
canvas.addEventListener('pointerdown',e=>{if(panel||!started||game.s.docked||game.surface)return;const p={x:(e.clientX-width/2)/cam.zoom+cam.x,y:(e.clientY-height/2)/cam.zoom+cam.y};const all=[...(game.sys.hasStation?[game.station]:[]),game.star,...game.wakes,...game.signals,...game.derelicts,...game.patrols,...game.traffic,...game.visiblePlanets,...game.enemies,...game.asteroids];const target=all.filter(t=>dist(p,t)<Math.max(t.r,35/cam.zoom)).sort((a,b)=>dist(p,a)-dist(p,b))[0];if(target){game.target=target;game.auto=null;updateHUD();}});
