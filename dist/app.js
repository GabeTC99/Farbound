...
function updateFrontierHUD(){
 ...
 const cataloged=s.systemScans.includes(s.system),discovery=$('discovery-button');discovery.hidden=!!surface||!!onfoot;discovery.disabled=s.docked||!!game.jump||!!game.scan||!!game.discoveryScan||cataloged; const shortLabels=compactUI();discovery.textContent=game.discoveryScan?(shortLabels?'CHARGING…':'PULSE CHARGING…'):cataloged?(shortLabels?'CATALOGED':'SYSTEM CATALOGED'):(shortLabels?'PULSE':'DISCOVERY PULSE');
 $('scanner-status').hidden=!!surface||!!onfoot||shortLabels;$('scanner-status').textContent=(cataloged?'Catalog complete':game.sys.uncharted?'Discovery pulse locates worlds':'Known stellar charts')+' · '+game.visiblePlanets.length+' worlds located · '+game.planets.filter(p=>s.scanned.includes(p.id)).length+'/'+game.planets.length+' surveyed';
