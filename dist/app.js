...
function sound(kind='click'){if(!game.s.sound||!started)return;try{engine.play(kind);}catch{}}
...
const stationArt=new Image();stationArt.decoding='async';stationArt.src='station.webp';
function drawStation(s=game.station){if(!s||s.type==='beacon')return;const scale=s.r/65;ctx.save();ctx.translate(s.x,s.y);ctx.rotate(clock*.06);ctx.scale(scale,scale);if(stationArt.complete&&stationArt.naturalWidth){ctx.drawImage(stationArt,-78,-78,156,156);const glow=ctx.createRadialGradient(0,0,2,0,0,15);glow.addColorStop(0,`rgba(118,239,219,${.32+.16*Math.sin(clock*2.4)})`);glow.addColorStop(1,'rgba(118,239,219,0)');ctx.fillStyle=glow;ctx.beginPath();ctx.arc(0,0,15,0,Math.PI*2);ctx.fill();}else{ctx.lineWidth=2;circle(0,0,66,'#324f5c');circle(0,0,64,'#527d8c',true);circle(0,0,51,'#223b47');circle(0,0,51,'#6a94a3',true);for(let i=0;i<6;i++){ctx.save();ctx.rotate(i*Math.PI/3);ctx.fillStyle='#18303e';ctx.strokeStyle='#6a94a3';ctx.fillRect(35,-9,45,18);ctx.strokeRect(35,-9,45,18);ctx.fillStyle='#82d7c2';ctx.fillRect(68,-3,7,6);ctx.restore();}circle(0,0,22,'#132833');circle(0,0,22,'#7db9bd',true);}ctx.restore();ctx.setLineDash([9,13]);circle(s.x,s.y,125*scale,'#4c8c8740',true);ctx.setLineDash([]);label(s.name,s.x,s.y-100*scale,'#94c7cd');label(s.roleLabel||'ORBITAL STATION',s.x,s.y-78*scale,'#608592',12);}
