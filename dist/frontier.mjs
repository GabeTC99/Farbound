...
import {speakHort,ensureHortState} from './station-hort.mjs';
...
export function pursuitObjective(game){
 const p=game.s?.pursuit;if(!p)return null;
 if(p.until&&(game.s.playtime||0)>p.until)return null;
 if(p.to===game.s.system)return{text:`Intercept ${p.name} before they dock`,kind:p.kind};
...
export {STATION_HORT,HORT_LINES,HORT_SYSTEM_ID,HORT_SYSTEM_NAME,HORT_SYSTEM_NICK,pickHortLine,speakHort,ensureHortState} from './station-hort.mjs';
export {buildSystemLayout,surveyWorldIds,systemLayoutMeta,isLandablePlanet,isLandableBody,PLANET_KINDS,PLANET_KIND_IDS,STAR_TYPES,STAR_CLASS_IDS,advanceOrbits,applyOrbitPhase,pickPlanetKind,findBodyOfKind,findStarOfClass} from './system-layout.mjs';
...
export function validateSave(x){
 ...
 try{const s=migrate(base);
  ...
  if(x.spectrumScanned!==undefined){if(!Array.isArray(x.spectrumScanned)||x.spectrumScanned.length>4096||x.spectrumScanned.some(id=>!/^(planet-\d+-\d+|moon-\d+-\d+-\d+)$/.test(id)))return null;s.spectrumScanned=[...new Set(x.spectrumScanned)];}else s.spectrumScanned=s.spectrumScanned||[];
  ...
  for(const k of ['discoveries','anomalies','geology','deliveries','operations','pirates','interventions','salvages','pursuits']){if(!numeric(x.metrics?.[k]??(k==='interventions'||k==='salvages'||k==='pursuits'?0:null)))return null;s.metrics[k]=x.metrics?.[k]??0;}for(const id of factionIds){if(!numeric(x.metrics?.factionKills?.[id]))return null;s.metrics.factionKills[id]=x.metrics.factionKills[id];}
  ...
  if(x.briefed!==undefined){if(typeof x.briefed!=='boolean')return null;s.briefed=x.briefed;}else s.briefed=!!s.robotMet;
  s.pursuit=null;
  if(x.pursuit!=null){
   const p=x.pursuit;
   if(!p||typeof p.uid!=='string'||p.uid.length>48||typeof p.name!=='string'||!p.name.length||p.name.length>60||typeof p.hull!=='string'||p.hull.length>24||!['trader','fugitive'].includes(p.kind)||!Number.isInteger(p.to)||!SYSTEMS[p.to])return null;
   s.pursuit={uid:p.uid,name:p.name,hull:p.hull,color:typeof p.color==='string'&&p.color.length<24?p.color:'#d2b48c',size:Number.isFinite(p.size)?clamp(p.size,8,32):14,from:Number.isInteger(p.from)&&SYSTEMS[p.from]?p.from:0,to:p.to,kind:p.kind,until:Number.isFinite(p.until)?p.until:0};
  }
  s.allegiance=factionIds.includes(x.allegiance)?x.allegiance:null;s.engineVolume=clamp(Number.isFinite(Number(x.engineVolume))?Number(x.engineVolume):.35,0,1);s.graphics=['high','balanced','performance'].includes(x.graphics)?x.graphics:'high';s.showFps=x.showFps===true;s.nextModule=Math.max(1,...s.modules.map(m=>+m.uid.slice(2)+1));s.nextOperation=Math.max(1,Number.isInteger(x.nextOperation)?x.nextOperation:1,...s.operations.map(o=>+o.uid.slice(3)+1));
...
const COMPANY_POOL={
 ...
 Extraction:[{name:'Helion Oreworks',focus:'mining'},{name:'Beltline Freight',focus:'delivery'},{name:'Deepcut Extraction',focus:'mining'}],
 Industrial:[{name:'Forgeworks Combine',focus:'delivery'},{name:'Lattice Fabrication',focus:'mining'},{name:'Haulright Logistics',focus:'delivery'}],
 Research:[{name:'Aperture Survey',focus:'survey'},{name:'Helix Laboratories',focus:'survey'},{name:'Lumen Security',focus:'bounty'}],
 Frontier:[{name:'Wayfarer Supply',focus:'delivery'},{name:'Rimward Freight',focus:'delivery'},{name:'Watchfall Security',focus:'bounty'}]
};
const CONTRACT_BAGS={
 Agricultural:['delivery','survey','delivery','mining'],
 Extraction:['mining','delivery','mining','delivery'],
 Industrial:['delivery','mining','bounty','delivery'],
 Research:['survey','delivery','survey','bounty'],
 Frontier:['bounty','delivery','mining','survey']
};
export const COMPANY_RANKS=[
 {level:0,name:'Associate',min:0,pay:1},
 {level:1,name:'Trusted',min:16,pay:1.25},
 {level:2,name:'Preferred',min:40,pay:1.5},
 {level:3,name:'Partner',min:72,pay:1.8}
];
export function companyRank(standing=0){let rank=COMPANY_RANKS[0];for(const r of COMPANY_RANKS)if(standing>=r.min)rank=r;return rank;}
export function companiesFor(sys){
 const pool=COMPANY_POOL[sys.eco]||COMPANY_POOL.Frontier,faction=FACTIONS.find(f=>f.id===sys.faction);
 return [0,1].map(i=>{const def=pool[(sys.id+i)%pool.length];return {id:`co-${sys.id}-${i===0?'a':'b'}`,name:def.name,focus:def.focus,faction:sys.faction||null,factionName:faction?.name||null};});
}
function issuerFor(companies,type,slot){const focused=companies.filter(c=>c.focus===type);return (focused.length?focused:companies)[slot%(focused.length||companies.length)];}
function nearbySystems(here,needStation){return SYSTEMS.filter(x=>x.id!==here.id&&(!needStation||x.hasStation!==false)).sort((a,b)=>dist(a,here)-dist(b,here));}
function makeContract(here,s,type,slot,gen,company,premium){
 const id=`${here.id}-${gen*10+slot}`,rank=companyRank(s.companies?.[company.id]?.standing||0),pay=rank.pay*(premium?1.2:1);
 const ports=nearbySystems(here,true),surveys=nearbySystems(here,false);
 const target=ports[slot%Math.max(1,ports.length)]||here,dest=surveys[(slot+1)%Math.max(1,surveys.length)]||here;
 const worlds=surveyWorldIds(dest.id,dest),worldWord=worlds.length===1?'world':`all ${worlds.length} worlds`;
 const faction=company.factionName,posted=`Posted by ${company.name}${faction?' for '+faction:''}.`,prefix=premium?'Priority · ':'';
...
export function contractsFor(s){
 const here=SYSTEMS[s.system];
 if(!here||here.hasStation===false)return [];
...
export class Game extends FlightGame{
 constructor(save=newSave()){super(save.version===1?migrate(save):save);this.surface=null;this.onfoot=null;this.scooping=false;this.discoveryScan=null;this.dynScan=null;this.wakes=[];this.signals=[];this.derelicts=[];this.responseAt=null;this.responseWave=0;this.heatWanted=0;this.lastKnown=null;this.lastHitSecurity=false;this.dyn=new DynamicEventManager(this);this.s.heat??=25;this.s.bounty??=0;this.s.detained??=false;this.s.robotMet??=false;this.s.companies??={};this.s.briefed??=false;this.s.pursuit??=null;this.s.stationPos??=null;this.s.systemScans??=[...this.s.visited];this.s.spectrumScanned??=[];this.s.explorationLog??=[];this.s.transit??=[];if(!Number.isFinite(this.s.engineVolume))this.s.engineVolume=.35;if(!['high','balanced','performance'].includes(this.s.graphics))this.s.graphics='high';this.s.showFps=this.s.showFps===true;ensureRobotState(this);ensureHortState(this);this.surfaceRecordsStart=this.s.records.length;if(this.s.surface){const p=this.planets.find(p=>p.id===this.s.surface.planetId);if(p){this.surface=createSurface(p,this.s.surfaceScanned,this.s.surface);this.surfaceRecordsStart=this.s.surface.recordStart;const ground=terrainAt(this.surface.x,this.surface.seed)-19;if(this.surface.y>=ground-2){this.surface.y=ground;this.surface.landed=true;}if(this.s.surface.foot&&this.surface.landed)this.onfoot=createOnFoot(createPlanetLayout(this.surface),this.s.surface.foot);}}else if(this.s.docked)this.enterStationDeck();}
 ...
 enterStationDeck(){
  ...
  const layout=createStationLayout({
   ...
   solace:this.s.system===0&&!this.sys.prison
  });
 ...
 ingestTransitArrivals(){
  ...
  for(const t of due){
   ...
   const pursued=this.s.pursuit?.uid===t.uid;
   this.traffic.push({id:'traffic-arr-'+t.uid,uid:t.uid,type:'traffic',name:t.name,job:'ARRIVING FROM JUMP POINT',hull:t.hull,color:t.color,size:t.size,turn:t.turn,speed:t.speed,wait:t.wait,canJump:true,pursued,points:[jump,station],x:jump.x,y:jump.y,angle:Math.atan2(station.y-jump.y,station.x-jump.x),r:Math.max(11,t.size-1),hp:48+t.size*2,max:48+t.size*2,target:1,pause:0.4,thrust:0,status:'AT JUMP POINT',underAttackUntil:0,phase:Math.random()*6,work:0,trail:[]});
  }
 }
 departJump(ship){
 ...
 updateTraffic(dt){
  if(this.s.pursuit?.until&&this.s.playtime>this.s.pursuit.until)this.clearPursuit('The contact folded too far ahead. Pursuit dropped.');
  ...
  for(const ship of [...this.traffic]){
   ...
   if(ship.pause>0){
    ...
    if(dist(ship,stop)<18){
     ...
     if(stop.status==='MINING RUN'||stop.status==='PLANETARY SURVEY'){
      ship.x=stop.x+Math.sin(this.time*1.4+ship.phase)*2.2;
      ship.y=stop.y+Math.cos(this.time*1.1+ship.phase)*1.6;
      ship.angle+=dt*.35;
     }else if(stop.status==='FUEL SCOOPING'){
      ship.x=stop.x+Math.sin(this.time*.6+ship.phase)*1.4;
      ship.y=stop.y+Math.cos(this.time*.5+ship.phase)*1.1;
      ship.angle=Math.atan2(this.star.y-ship.y,this.star.x-ship.x);
     }else{ship.x=stop.x;ship.y=stop.y;}
    ...
    if(ship.pause<=0&&ship.canJump&&stop.status==='DOCKED'&&ship.job==='ARRIVING FROM JUMP POINT'){
     if(ship.pursued)this.clearPursuit(`${ship.name} reached the station. Pursuit lost.`);
   ...
   if(ship.thrust>.08&&(this.s.graphics||'high')==='high'){
 ...
 scanWake(){
  ...
  this.notify(`${wake.shipName} jumped toward ${systemName(SYSTEMS[wake.to],this.s)}. Plot the wake to intercept.`,'good');
  return true;
 }
 followWake(){
  const wake=this.target?.type==='wake'?this.target:null;
  if(!wake){this.notify('Target a scanned wake first.');return false;}
  if(!wake.scanned)return this.scanWake();
  const kind=wake.hull==='courier'&&Math.random()<.4?'fugitive':'trader';
  this.s.pursuit={uid:wake.uid,name:wake.shipName,hull:wake.hull||'freighter',color:wake.color||'#d2b48c',size:wake.size||14,from:wake.from,to:wake.to,kind,until:(this.s.playtime||0)+2400};
  const t=this.s.transit.find(x=>x.uid===wake.uid);
  if(t)t.eta=this.s.playtime;
  this.notify(`Pursuit logged · ${wake.shipName} bound for ${systemName(SYSTEMS[wake.to],this.s)}. Jump to intercept.`,'good');
  return this.setRoute(wake.to);
 }
 prospectRock(){
  if(this.s.docked||this.surface||this.jump){this.notify('Launch into local space to prospect.');return false;}
  const rock=this.target?.type==='asteroid'?this.target:this.asteroids.filter(a=>dist(this.player,a)<220).sort((a,b)=>dist(this.player,a)-dist(this.player,b))[0];
  if(!rock){this.notify('Target a rock to prospect it.');return false;}
  if(Math.hypot(this.player.vx,this.player.vy)>140){this.notify('Slow below 140 m/s to prospect.');return false;}
  if(dist(this.player,rock)>220){this.notify('Approach the rock to prospect it.');return false;}
  this.target=rock;rock.prospected=true;
  const label=rock.ore==='crystal'?'Void crystals':'Titanium ore';
  this.notify(`Prospected · ${label}. Mining this rock yields extra.`,'good');
  return true;
 }
 scanPursuit(){
  const ship=this.target?.pursued?this.target:this.traffic.find(t=>t.pursued&&dist(this.player,t)<280);
  if(!ship){this.notify('No pursued contact in range.');return false;}
  if(Math.hypot(this.player.vx,this.player.vy)>160){this.notify('Slow down to scan the contact.');return false;}
  if(dist(this.player,ship)>280){this.notify('Close to 280 m to scan the contact.');return false;}
  const p=this.s.pursuit;
  this.s.metrics.pursuits=(this.s.metrics.pursuits||0)+1;
  if(p?.kind==='fugitive'){
   ship.type='enemy';ship.bounty=520;ship.wanted=true;ship.hp=ship.hp||90;ship.max=ship.max||90;ship.r=ship.r||18;ship.fire=.4;ship.eventOwned=true;
   this.enemies.push(ship);this.traffic=this.traffic.filter(t=>t!==ship);this.target=ship;this.s.pursuit=null;
   this.notify(`${ship.name} flagged wanted · ${ship.bounty} cr bounty.`,'good');
   return true;
  }
  const goods=['food','ore','tech','meds','crystal'],good=goods[Math.floor(Math.random()*goods.length)];
  this.s.credits+=180;
  let extra='';
  const st=getStats(this.s),free=st.cargo-cargoUsed(this.s);
  if(free>0){this.s.cargo[good]=(this.s.cargo[good]||0)+1;extra=` · +1 t ${good}`;}
  this.notify(`Manifest scanned · ${ship.name} hauling ${good} · +180 cr${extra}`,'good');
  this.s.pursuit=null;ship.pursued=false;
  return true;
 }
 realizePursuit(){
  const p=this.s.pursuit;if(!p)return false;
  if(p.until&&this.s.playtime>p.until){this.clearPursuit();return false;}
  if(p.to!==this.s.system)return false;
  let ship=this.traffic.find(t=>t.uid===p.uid)||this.traffic.find(t=>t.pursued);
  if(!ship){
   const jump=this.jumpAnchor(40);
   const dock=this.sys.hasStation?{x:this.station.x,y:this.station.y,status:'DOCKED'}:{x:jump.x+420,y:jump.y+80,status:'AT JUMP POINT'};
   ship={id:'traffic-pursuit',uid:p.uid,type:'traffic',name:p.name,job:'ARRIVING FROM JUMP POINT',hull:p.hull,color:p.color,size:p.size,turn:2.4,speed:118,wait:3.4,canJump:true,pursued:true,points:[jump,dock],x:jump.x,y:jump.y,angle:Math.atan2(dock.y-jump.y,dock.x-jump.x),r:Math.max(11,(p.size||14)-1),hp:52+(p.size||14)*2,max:52+(p.size||14)*2,target:1,pause:0.3,thrust:0,status:'AT JUMP POINT',underAttackUntil:0,phase:Math.random()*6,work:0,trail:[]};
   this.traffic.push(ship);
  }
  ship.pursued=true;this.target=ship;
  const key=p.uid+'@'+this.s.system;
  if(this._pursuitAnnounced!==key){
   this._pursuitAnnounced=key;
   this.notify(p.kind==='fugitive'?`Fugitive contact · ${p.name} at the jump point. Scan or intercept.`:`Wake contact · ${p.name} dropping in. Scan their hold before they dock.`,'good');
  }
  return true;
 }
 clearPursuit(reason){
  const p=this.s.pursuit;if(!p)return;
  for(const t of this.traffic)if(t.pursued||t.uid===p.uid)t.pursued=false;
  this.s.pursuit=null;this._pursuitAnnounced=null;
 ...
 talkHort(mode='talk'){if(!this.s.docked||this.s.system!==0){this.notify('Hort keeps the Solace messenger desk.');return null;}return speakHort(this,mode==='tip'?'tip':mode==='greeting'?'greeting':'talk');}
 ...
 discover(){
  ...
  if(this.discoveryScan||this.scan||this.spectrumScan){this.notify('Scanner already active.');return false;}
  if(this.s.systemScans.includes(this.s.system)){this.notify('System catalog complete. Spectrum-scan worlds from range, then approach for a detailed survey.');return false;}
  this.discoveryScan={progress:0};this.notify('Discovery pulse charging.');return true;
 }
 scanTarget(){
  if(this.surface)return this.scanSurface();
  if(this.discoveryScan||this.jump)return false;
 ...
 takeoff(){if(!this.surface)return false;if(this.onfoot){this.notify('Board the skiff first.');return false;}const p=this.planets.find(p=>p.id===this.surface.planetId);this.endPlanetAmbience();this.surface=null;this.s.surface=null;this.onfoot=null;this.player.x=p.x;this.player.y=p.y+p.r+360;this.player.vx=this.player.vy=0;this.target=p;this.beginSiteTransition('takeoff');this.notify('Back in orbit. Dock to sell anomaly signals.');return true;}
 ...
 boardSkiff(){
  ...
  this.beginSiteTransition('embark');
  this.notify('Back aboard the skiff.');
  return true;
 }
 beginSiteTransition(kind){
  this.siteFx=createSiteTransition(kind);
  this.fireAudioCue(SURFACE_AUDIO_CUES[kind]||SURFACE_AUDIO_CUES.embark);
  return this.siteFx;
 }
 fireAudioCue(name){
  if(!name)return;
  (this.audioCues||=[]).push(name);
  if(this.audioCues.length>16)this.audioCues.shift();
 }
 fireAudioStop(name){
  if(!name)return;
  (this.audioStops||=[]).push(name);
  if(this.audioStops.length>8)this.audioStops.shift();
 }
 beginPlanetAmbience(){
  const cue=surfaceAmbientCue(this.onfoot?.kindId||this.surface?.kindId);
  if(this.siteAmbient&&this.siteAmbient!==cue)this.fireAudioStop(this.siteAmbient);
  this.siteAmbient=cue||null;
  if(cue)this.fireAudioCue(cue);
 }
 endPlanetAmbience(){
  if(!this.siteAmbient)return;
 ...
 advanceSiteTransition(dt){
  if(!this.siteFx)return false;
  this.siteFx.t=(this.siteFx.t||0)+dt;
  if(siteTransitionDone(this.siteFx))this.siteFx=null;
  return !!this.siteFx;
 }
 interactPlanet(){
  ...
  if(z.service==='inspect'&&z.anomalyId){
   ...
   if(!this.onfoot.footScan||this.onfoot.footScan.id!==z.id){
    ...
    this.fireAudioCue(SURFACE_AUDIO_CUES.inspectStart);
    this.fireAudioCue(SURFACE_AUDIO_CUES.inspectLoop);
 ...
 updatePlanetFootScan(dt){
  ...
  if(!z||Math.hypot(z.x-this.onfoot.x,z.y-this.onfoot.y)>(z.r||40)+8){this.onfoot.footScan=null;this.fireAudioCue(SURFACE_AUDIO_CUES.inspectStop);this.notify('Survey interrupted.');return;}
  scan.progress+=dt;
  if(scan.progress<scan.need)return;
  this.onfoot.footScan=null;
  this.fireAudioCue(SURFACE_AUDIO_CUES.inspectStop);
 ...
 rescue(){this.endPlanetAmbience();this.surface=null;this.s.surface=null;this.onfoot=null;this.s.stationPos=null;this.s.records=[];this.s.explorationLog=this.s.explorationLog.filter(e=>e.sold);this.s.system=this.nearestPort();this.s.route=null;super.rescue();this.s.heat=25;if(this.s.docked)this.enterStationDeck();}
 update(dt,input={}){
  dt=clamp(dt,0,.05);
  this.advanceSiteTransition(dt);
  ...
  if(previous!==this.s.system){
   this.refreshRoute();
   if(wasJumping&&!this.s.docked){this.player.x=this.star.x+this.star.r+950;this.player.y=this.star.y;this.player.angle=0;this.player.vx=this.player.vy=0;this.target=this.star;const chased=this.realizePursuit();if(!chased)this.notify('Arrived near the primary star. Run a discovery scan to catalog this system.');}
  }
  if(this.s.docked){
