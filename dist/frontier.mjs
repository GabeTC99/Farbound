...
export class Game extends FlightGame{
 ...
 makeTraffic(){
  ...
  return jobs.map((j,i)=>{const start=j.points[0],next=j.points[1];return{...j,id:'traffic-'+i,type:'traffic',x:start.x+i*18,y:start.y+i*12,angle:Math.atan2(next.y-start.y,next.x-start.x),r:12,hp:55,max:55,target:1,pause:i*.7,thrust:0,status:'TRANSIT',underAttackUntil:0};});
 }
 updateTraffic(dt){
  for(const ship of this.traffic){
   if(ship.pause>0){ship.pause-=dt;ship.thrust=0;ship.status=ship.target===0?ship.job:'DOCKED';}
   else{const target=ship.points[ship.target],dx=target.x-ship.x,dy=target.y-ship.y,d=Math.hypot(dx,dy);if(d<18){ship.x=target.x;ship.y=target.y;ship.pause=ship.target===0?2:ship.wait;ship.status=ship.target===0?'DOCKED':ship.job;ship.target=ship.target?0:1;ship.thrust=0;}else{ship.angle=Math.atan2(dy,dx);const speed=Math.min(ship.speed,Math.max(24,d*.55));ship.x+=Math.cos(ship.angle)*speed*dt;ship.y+=Math.sin(ship.angle)*speed*dt;ship.thrust=Math.min(1,speed/ship.speed);ship.status='IN TRANSIT';}}
