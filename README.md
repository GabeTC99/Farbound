...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-ships2` so Pages testers pick up the seven hull plates plus Wren, Sparrow, and Kestrel thruster, flyby, and land MP3s. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **Hull plates:** `dist/assets/ships/` covers Wren, Sparrow, Kestrel, Mule, Rook, Tern, and Jackal. Flight and the hangar card use the PNG when it loads. A missing PNG keeps the current silhouette.
- **Hull cues:** Wren, Sparrow, and Kestrel play `thruster_*` while thrusting, `flyby_*` on boost, and `land_*` on touchdown. Mule, Rook, Tern, and Jackal keep the oscillator bed and the embark whoosh until their MP3s land. Other hulls stay on the oscillator.
