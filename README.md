...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-onfoot3` so Pages testers pick up the JS and planetary MP3s. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
...
- **Kind SFX:** On-foot loops `ambient_<kindId>` for metal / mineral / icegiant and plays throttled `grit_<kindId>` footfalls (including **grit_mineral**, Gabe-approved). Other kinds stay silent on those hooks. Files live under `dist/assets/audio/planetary/` and are in the service-worker `FILES` list.
