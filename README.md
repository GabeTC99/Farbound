...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-onfoot4` so Pages testers pick up the JS and planetary MP3s. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
...
- **On-foot ground:** Site floor tiles and near hills scroll with `siteWorldScroll` so they stay planted while the walker moves. Skiff, pads, and rocks keep the same camera. LAND climb lock is unchanged.
