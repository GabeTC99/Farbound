...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-space9` so Pages testers pick up thrust and cruise on the plated thruster loop, a silent space oscillator, and a full-bleed hangar plate, on top of the sparse station light masks (bay idle and flash, beacon B, window glow) on the single station silhouette, on top of the soft ion thruster loops for all 20 hulls, the spinning station, and the music beds, including the earlier Wren and Sparrow thruster re-rolls, the clean hull nose, the brake pitch lock, the outer-space audio bed, and the space plate stack, the quieter ship bus, NPC hull cues, all 20 plates, all 60 thruster, flyby, and land MP3s, and the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **Outer space:** Flight view layers `dist/assets/space/` back to front: far stars, a nebula wash, mid stars, a station plate by distance, dust, then a warm sun disc, bloom, and streak. The old procedural starfield stays only until the far plate loads. Near a station the exterior shows one `station_rot_00`–`07` frame at a time (no crossfade). The hangar plate covers the viewport, including tall phones, so the empty side margins of the plate stay off screen. Sparse masks blink on top: bay idle and flash, beacon A and B, a nav pulse, and a soft window glow. Performance mode skips those overlays. Thrust and cruise play `thruster_<hull>.mp3`; boost still fires the flyby. The space oscillator stays silent so brake and cruise do not whistle. In local space a quiet `vacuum_bed` loops under the ship thrust bus, with sparse `vacuum_radio` ticks, a rising `station_approach` hum inside the approach band, and a single `station_dock` confirm on docking. `music_flight` loops in space and crossfades to `music_station` as you close or dock, ducked under those space cues. Landing, on-foot vistas, grit, ship cues, and the clean hull nose are unchanged.
