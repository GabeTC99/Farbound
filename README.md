...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-space1` so Pages testers pick up the outer-space plate stack on top of the quieter ship bus, NPC hull cues, all 20 plates, all 60 thruster, flyby, and land MP3s, and the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **Outer space:** Flight view layers `dist/assets/space/` back to front: far stars, a nebula wash, mid stars, a station plate by distance, dust, then a warm sun disc, bloom, and streak. The old procedural starfield stays only until the far plate loads. Landing and on-foot vistas are unchanged.
