...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-ships1` so Pages testers pick up Wren, Sparrow, and Kestrel side plates plus their thruster, flyby, and land MP3s. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **Hull plates:** `dist/assets/ships/{wren,sparrow,kestrel}.png` replace procedural paint in flight and the hangar card when the file loads. A missing PNG keeps the current silhouette.
- **Hull cues:** `thruster_*` loops while that hull thrusts in space. `flyby_*` plays on boost. `land_*` plays on touchdown. Other hulls keep the oscillator bed.
