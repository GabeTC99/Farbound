...
- **Planetary landings:** Surface flight is no longer a flat silhouette. Sunlit slope terrain, planet-albedo ground tint, aerial-perspective hill bands, approach haze, a volumetric skiff, and a readable ALT / SINK / SIGNALS instrument strip. On-foot pads get a layered horizon, sun disc, marked ellipse, and 2.5D rocks.
...
- **Art hooks:** `dist/assets/new-frontier/` plus `loadFrontierImage()` for later Scenario Pro PNGs. Phase 1 is in-engine only — no placeholder sprites.
