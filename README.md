...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-space14` so Pages testers pick up a world-scaled station hull drawn above the dust (no hangar plate swap in flight), seam-free sky layers (vignetted nebula, mid-star and dust plates draw once, oversized, instead of tiling into dark bands), a fast station concourse (the static deck floor is rasterized once into cached 512px tiles and the backdrop into one cached image, so a frame is a few blits plus the crew; about 5x faster than the previous build in a headless phone-size bench), correct station depth (the hub rim sits under the arms, only viewer-facing rim windows draw, gates and arm walls are depth-sorted with the crew, near-side arm walls are knee-high cutaways and gate portals are glass), the FPS meter moved off the HUD cards, synthesized NPC flyby (doppler pass) and landing (touchdown and spool-down) cues plus a new station approach bed in the same family as the thrusters, a rebuilt station interior (synthesized concourse ambience plus a hangar bed that swells toward the launch arm, metal deck footsteps on walk cadence, a desk chime per visit and the hangar door at the launch pad, console ticks for UI taps; the old 88/132 Hz oscillator hum is retired; generated steel deck plating projected onto the iso deck, steel wall palette, and the real star plates behind the concourse), a rebuilt engine: all 20 thruster loops are synthesized seamless loops (soft engine rush plus a warm harmonic hum, pitched per hull class) and the player loop runs through a live voice whose level, lowpass and pitch follow thrust, speed and boost with smooth fades instead of hard start/stop clicks, NPC thrusters capped at two well under the player's, gapless thruster loop points, a continuously rotating orthographic station, thrust and cruise on the plated thruster loop, a silent space oscillator, and a full-bleed hangar plate, on top of the soft ion thruster loops for all 20 hulls, the spinning station, and the music beds, including the earlier Wren and Sparrow thruster re-rolls, the clean hull nose, the brake pitch lock, the outer-space audio bed, and the space plate stack, the quieter ship bus, NPC hull cues, all 20 plates, all 60 thruster, flyby, and land MP3s, and the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **Outer space:** Flight view layers `dist/assets/space/` back to front: far stars, a nebula wash, mid stars, a station plate by distance, dust, then a warm sun disc, bloom, and streak. The old procedural starfield stays only until the far plate loads. Near a station the exterior and approach draw one `station_ortho` sprite, spun with `ctx.rotate()` around its center. Performance mode still spins, more slowly. The hangar plate covers the viewport, including tall phones, so the empty side margins of the plate stay off screen. Thrust and cruise play `thruster_<hull>.mp3`; boost still fires the flyby. The space oscillator stays silent so brake and cruise do not whistle. In local space a quiet `vacuum_bed` loops under the ship thrust bus, with sparse `vacuum_radio` ticks, a rising `station_approach` hum inside the approach band, and a single `station_dock` confirm on docking. `music_flight` loops in space and crossfades to `music_station` as you close or dock, ducked under those space cues. Landing, on-foot vistas, grit, ship cues, and the clean hull nose are unchanged.
- **On-foot grit:** Foot texture is a sparse oneshot, not a stacked crack. The previous grit source stops before the next starts. Ice, ice giant, volcanic, and metal wait longer and sit quieter than the other kinds. Sprint steps a little faster than a walk. Ambient and inspect stay at the shared cue level. `grit_ice.mp3` is the soft snow-crunch re-roll.
- **Hull plates:** `dist/assets/ships/` covers every player hull. Flight and the hangar card use the PNG when it loads. A missing PNG keeps the current silhouette.
- **Hull cues:** Every player hull plays `thruster_*` while thrusting, `flyby_*` on boost, and `land_*` on touchdown. Those MP3s sit on a ship bus under the shared cue level (thruster about 0.25, flyby and land about 0.42). Nearby NPC traffic, patrols, and pirates use the same plated stems, distance-attenuated, with at most three thruster loops. A hull with no file yet falls back to the oscillator bed and the embark whoosh.
...
- **Embark / takeoff:** Boarding the skiff or returning to orbit starts a short fade-in veil (`createSiteTransition`) plus `surface.embark` / `surface.takeoff` (same `embark_whoosh.mp3`).
...
- **Inspect cues:** Pad survey fires `surface.inspect.start` / `.loop` / `.stop`, bound to `pad_inspect_*.mp3`. Embark/takeoff share `embark_whoosh.mp3`.
- **Kind SFX:** On-foot loops `ambient_<kindId>` and plays throttled `grit_<kindId>` for every planet kind (earthlike, ocean, arid, ice, metal, mineral, volcanic, barren, toxic, gas, icegiant). Inspect and embark/takeoff play on every kind. Files live under `dist/assets/audio/planetary/` and are decoded through the unlocked AudioContext.
- **On-foot ground:** Distant hills stay screen-stable (no scroll sampling). The floor tile shifts in X only, and the site camera Y is locked to the skiff ground line so rocks, pads, and the lander cannot climb into the sky. Walker Y is clamped to that band. LAND climb lock is unchanged.
