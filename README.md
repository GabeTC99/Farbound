...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-grit1` so Pages testers pick up the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
- **On-foot grit:** Foot texture is a sparse oneshot, not a stacked crack. The previous grit source stops before the next starts. Ice, ice giant, volcanic, and metal wait longer and sit quieter than the other kinds. Sprint steps a little faster than a walk. Ambient and inspect stay at the shared cue level.
...
- **Hull cues:** Wren through Jackal play `thruster_*` while thrusting, `flyby_*` on boost, and `land_*` on touchdown. The other 13 plated hulls keep the oscillator bed and the embark whoosh until their MP3s land.
