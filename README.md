...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-space3` so Pages testers pick up the clean hull nose and the brake pitch lock on top of the outer-space audio bed and the space plate stack, the quieter ship bus, NPC hull cues, all 20 plates, all 60 thruster, flyby, and land MP3s, and the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
