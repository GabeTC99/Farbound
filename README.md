...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-ships3` so Pages testers pick up the seven hull plates and all 21 thruster, flyby, and land MP3s. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
...
- **Hull cues:** Wren, Sparrow, Kestrel, Mule, Rook, Tern, and Jackal play `thruster_*` while thrusting, `flyby_*` on boost, and `land_*` on touchdown. A plated hull with no file yet falls back to the oscillator bed and the embark whoosh. Other hulls stay on the oscillator.
