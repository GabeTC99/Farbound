...
- **Ship:** Player-facing version stays **3.0.0**. Service worker cache is `farbound-v3.0.0-ships5` so Pages testers pick up all 20 hull plates, all 60 thruster, flyby, and land MP3s, and the softer on-foot grit. Android `versionCode` stays **13**. Do **not** tag Android or Windows Releases.
...
- **Hull cues:** Every player hull plays `thruster_*` while thrusting, `flyby_*` on boost, and `land_*` on touchdown. A hull with no file yet falls back to the oscillator bed and the embark whoosh.
