...
test('Station desks return to the deck and version comes from release.mjs',()=>{
 ...
 assert.match(release,/export const RELEASE_EDITION='New Frontier'/);
 assert.match(app,/import \{RELEASE,RELEASE_NAME,RELEASE_EDITION\} from '\.\/release\.mjs'/);
 ...
 assert.match(sw,/farbound-v3\.0\.0-space12/);
...
test('Player-facing product strings are Nullharbor; save keys stay farbound',()=>{
 ...
 assert.match(app,/NULLHARBOR · \$\{RELEASE_EDITION\} · v\$\{RELEASE\}/);
 ...
 assert.match(html,/Nullharbor · New Frontier/);
