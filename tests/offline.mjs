...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v3.0.0-space1');assert.equal(cache.size,188);
...
console.log('PASS Offline cache serves all 188 release and classic assets without network');
