...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v2.16.10');assert.equal(cache.size,50);
...
console.log('PASS Offline cache serves all 50 release and classic assets without network');
