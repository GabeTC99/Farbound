...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v2.12.0');assert.equal(cache.size,41);
...
assert.equal(calls.length,0);let status;await event('message',{data:{type:'CACHE_STATUS'},ports:[{postMessage:message=>status=message}]});assert(status.ready);assert.equal(status.release,'2.12.0');
