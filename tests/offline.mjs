...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v2.16.10');assert.equal(cache.size,51);
...
assert.equal(calls.length,0);let status;await event('message',{data:{type:'CACHE_STATUS'},ports:[{postMessage:message=>status=message}]});assert(status.ready);assert.equal(status.release,'2.16.10');
skip=false;await event('message',{data:{type:'SKIP_WAITING'}});assert(skip);
console.log('PASS Offline cache serves all 51 release and classic assets without network');
console.log('PASS Service worker activates only after caching the release and reports offline readiness');
let checked=0;async function inspect(dir){for(const file of await readdir(dir,{withFileTypes:true})){const abs=path.join(dir,file.name);if(file.isDirectory()){await inspect(abs);continue;}if(/\.(mjs|js)$/.test(file.name)){execFileSync(process.execPath,['--check',abs]);const src=await readFile(abs,'utf8');if(file.name.endsWith('.js')&&/^\s*import\s/m.test(src)){const tmp=path.join(os.tmpdir(),'farbound-esm-'+file.name.replace(/\W/g,'_')+'.mjs');await writeFile(tmp,src);try{execFileSync(process.execPath,['--check',tmp]);}finally{await unlink(tmp);} }for(const [,relative]of src.matchAll(/(?:from\s*|import\s*)['"](\.\.?\/[^'"]+)['"]/g))assert((await stat(path.resolve(dir,relative))).isFile(),relative);checked++;}if(file.name.endsWith('.webmanifest'))JSON.parse(await readFile(abs,'utf8'));}}
