...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v3.0.0-ships4');assert.equal(cache.size,137);
...
console.log('PASS Offline cache serves all 137 release and classic assets without network');
