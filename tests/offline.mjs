...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v3.0.0-space2');assert.equal(cache.size,192);
...
console.log('PASS Offline cache serves all 192 release and classic assets without network');
