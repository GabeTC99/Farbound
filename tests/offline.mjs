...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v3.0.0-ships2');assert.equal(cache.size,112);
...
console.log('PASS Offline cache serves all 112 release and classic assets without network');
