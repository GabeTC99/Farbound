...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v2.2.1');assert.equal(cache.size,39);
...
console.log('PASS Offline cache serves all 39 release and classic assets without network');
