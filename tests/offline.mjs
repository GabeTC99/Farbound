...
await event('install');assert(skip);await event('activate');assert(claimed);assert(!stores.has('farbound-v1.0.0'));assert(stores.has('unrelated-cache'));const cache=stores.get('farbound-v3.0.0-onfoot3');assert.equal(cache.size,80);
...
console.log('PASS Offline cache serves all 80 release and classic assets without network');
