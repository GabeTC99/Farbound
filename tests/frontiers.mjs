...
test('Planetary landings restore in place, record signals once, and sell only when docked',()=>{
 let g=new Game();g.launch();const p=g.planets[0];approach(g,p);ticks(g,1);assert(g.land());assert(!g.jumpTo(1));const a=g.surface.anomalies[0],initial=g.s.credits;scanSignal(g,a);assert.equal(g.s.records.length,1);assert.equal(g.s.metrics.anomalies,1);assert.equal(g.s.credits,initial);g=roundtrip(g);assert(g.surface);assert.equal(g.surface.anomalies[0].scanned,true);assert.equal(g.s.records.length,1);const value=g.s.records[0].value;g.takeoff();g.player.x=0;g.player.y=185;assert(g.dock());assert.equal(g.s.credits,initial+value);assert.equal(g.s.records.length,0);g.dock();assert.equal(g.s.credits,initial+value);g.launch();g.player.x=p.x;g.player.y=p.y+p.r+300;assert(g.land());assert(g.surface.anomalies[0].scanned);assert.equal(g.s.metrics.anomalies,1);
});
