...
OUT = ROOT / 'dist' / 'Nullharbor-Android-Source.zip'
...
with ZipFile(OUT, 'w', compression=ZIP_DEFLATED, compresslevel=9) as archive:
    for p in sorted(files):
        info = ZipInfo('Nullharbor/' + p.relative_to(ROOT).as_posix(), (2026, 9, 7, 0, 0, 0))
        info.compress_type = ZIP_DEFLATED
        info.external_attr = 0o644 << 16
        archive.writestr(info, p.read_bytes())
with ZipFile(OUT) as archive:
    assert archive.testzip() is None
    assert 'Nullharbor/dist/frontier.mjs' in archive.namelist()
    assert 'Nullharbor/dist/classic/core.mjs' in archive.namelist()
    assert 'Nullharbor/android/app/src/main/java/com/farbound/game/MainActivity.java' in archive.namelist()
